...
def get_jlink_dll(inpath=""):
    ...
    search_path += [os.path.join(inpath)]
...
import time
class JLink(object):
    def __init__(self, device = "QN9080C",inpath=""):
    # def __init__(self, device = "CORTEX-M4"):
        timeout=10
        retried=False
        t0=time.time()
        elapsed=-1
        ...
        while time.time() < t0+timeout:
            self.jl,self.jlink_lib_name = get_jlink_dll(inpath)
            try:
                self._init(device)
    ...
    def _init(self, device):
        self.tif_select(1) # crashes with EVB
        self.set_speed(10)
        self.exec_command('ProjectFile = JLinkSettings.ini')
        cmdstr = "Device = %s\n" % device
        # print cmdstr
        # self.exec_command("Device = QN9080C") # works with DK, but fails with EVB        
        # self.exec_command("Device = CORTEX-M4") # works with EVB
        self.exec_command(cmdstr)
        # self.reset() # mmerlin: remove reset
        pass
    ...
    @check_err
    def tif_select(self, tif): return self.jl.JLINKARM_TIF_Select(tif)
    @check_err
    def set_speed(self, khz): return self.jl.JLINKARM_SetSpeed(khz)
    @check_err  
    def exec_command(self, cmd): return self.jl.JLINKARM_ExecCommand(cmd)
    @check_err
