...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:xalan="http://xml.apache.org/xalan" xmlns:exslt="http://exslt.org/common" exclude-result-prefixes="xalan exslt msxsl" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:kactus2="http://funbase.cs.tut.fi/">
	...
	<xsl:template name="exslt:parse">
		...
		<xsl:variable name="valid">
			<xsl:call-template name="exslt:validate">
				<xsl:with-param name="tokens" select="$tokens"/>
			</xsl:call-template>
		</xsl:variable>
		<xsl:choose>
			<xsl:when test="not(contains($valid, 'false'))">
				<xsl:call-template name="exslt:replace">
					<xsl:with-param name="tokens" select="$tokens"/>
				</xsl:call-template>
			</xsl:when>
			<xsl:otherwise>
	...
	<xsl:template name="exslt:validate">
		<xsl:param name="tokens"/>
		<xsl:if test="$debug">
			<xsl:message>validate("<xsl:value-of select="$tokens"/>") [<xsl:value-of select="count(exslt:node-set($tokens)/token)"/>]</xsl:message>
			<xsl:message>
				<token>
					<xsl:attribute name="type"><xsl:value-of select="exslt:node-set($tokens)/token[1]/@type"/></xsl:attribute>
					<xsl:value-of select="exslt:node-set($tokens)/token[1]"/>
				</token>
			</xsl:message>
		</xsl:if>
		<xsl:if test="count(exslt:node-set($tokens)/token) > 0">
			<xsl:variable name="token" select="exslt:node-set($tokens)/token[1]"/>
			<xsl:choose>
				<xsl:when test="$token/@type = 'string'">
					<!-- Continue -->
					<xsl:call-template name="exslt:validate">
						<xsl:with-param name="tokens">
							<xsl:for-each select="exslt:node-set($tokens)/token[position() > 1]">
								<xsl:copy-of select="."/>
							</xsl:for-each>
						</xsl:with-param>
					</xsl:call-template>
				...
				<xsl:when test="$token = 'number' and exslt:node-set($tokens)/token[2] = '(' and exslt:node-set($tokens)/token[3] = ')'">
	...
	<xsl:template name="exslt:replace">
		<xsl:param name="tokens"/>
		<xsl:if test="$debug">
			<xsl:message>replace("<xsl:value-of select="exslt:node-set($tokens)"/>") [<xsl:value-of select="count(exslt:node-set($tokens)/token)"/>]</xsl:message>
			<xsl:message>
				<token>
					<xsl:attribute name="type"><xsl:value-of select="exslt:node-set($tokens)/token[1]/@type"/></xsl:attribute>
					<xsl:value-of select="exslt:node-set($tokens)/token[1]"/>
		...
		<xsl:if test="count(exslt:node-set($tokens)/token) > 0">
			<xsl:choose>
				<xsl:when test="exslt:node-set($tokens)/token[1]/@type = 'string'">
					<xsl:value-of select="exslt:node-set($tokens)/token[1]"/>
					<xsl:call-template name="exslt:replace">
						<xsl:with-param name="tokens">
							<xsl:for-each select="exslt:node-set($tokens)/token[position() > 1]">
				...
				<xsl:when test="exslt:node-set($tokens)/token[1] = 'true'">
					<xsl:text>1</xsl:text>
					<xsl:call-template name="exslt:replace">
						<xsl:with-param name="tokens">
							<xsl:for-each select="exslt:node-set($tokens)/token[position() > 3]">
								<xsl:copy-of select="."/>
							</xsl:for-each>
						</xsl:with-param>
					</xsl:call-template>
				</xsl:when>
				<xsl:when test="exslt:node-set($tokens)/token[1] = 'false'">
					<xsl:text>0</xsl:text>
					<xsl:call-template name="exslt:replace">
						<xsl:with-param name="tokens">
							<xsl:for-each select="exslt:node-set($tokens)/token[position() > 3]">
				...
				<xsl:when test="exslt:node-set($tokens)/token[1] = 'and'">
					<xsl:text> &amp;&amp; </xsl:text>
					<xsl:call-template name="exslt:replace">
						<xsl:with-param name="tokens">
							<xsl:for-each select="exslt:node-set($tokens)/token[position() > 1]">
				...
				<xsl:when test="exslt:node-set($tokens)/token[1] = 'div'">
					<xsl:text> / </xsl:text>
					<xsl:call-template name="exslt:replace">
						<xsl:with-param name="tokens">
							<xsl:for-each select="exslt:node-set($tokens)/token[position() > 1]">
				...
				<xsl:when test="exslt:node-set($tokens)/token[1] = 'mod'">
					<xsl:text> % </xsl:text>
					<xsl:call-template name="exslt:replace">
						<xsl:with-param name="tokens">
							<xsl:for-each select="exslt:node-set($tokens)/token[position() > 1]">
				...
				<xsl:when test="exslt:node-set($tokens)/token[1] = 'id'">
					<xsl:value-of select="exslt:node-set($tokens)/token[4]"/>
					<xsl:call-template name="add-space">
						<xsl:with-param name="next-token" select="exslt:node-set($tokens)/token[7]"/>
					</xsl:call-template>
					<xsl:call-template name="exslt:replace">
						<xsl:with-param name="tokens">
							<xsl:for-each select="exslt:node-set($tokens)/token[position() > 6]">
								<xsl:copy-of select="."/>
							</xsl:for-each>
						</xsl:with-param>
					</xsl:call-template>
				</xsl:when>
				<xsl:when test="exslt:node-set($tokens)/token[1] = 'boolean'">
				...
				<xsl:when test="exslt:node-set($tokens)/token[1] = 'not'">
					<xsl:text>!</xsl:text>
					<xsl:call-template name="exslt:replace">
						<xsl:with-param name="tokens">
							<xsl:for-each select="exslt:node-set($tokens)/token[position() > 1]">
				...
				<xsl:when test="exslt:node-set($tokens)/token[1] = 'number'">
				...
				<xsl:when test="exslt:node-set($tokens)/token[1] = 'floor'">
				...
				<xsl:when test="exslt:node-set($tokens)/token[1] = 'ceiling'">
				...
				<xsl:when test="exslt:node-set($tokens)/token[1] = 'round'">
				...
				<xsl:when test="exslt:node-set($tokens)/token[1] = '='">
					<xsl:text> == </xsl:text>
					<xsl:call-template name="exslt:replace">
						<xsl:with-param name="tokens">
							<xsl:for-each select="exslt:node-set($tokens)/token[position() > 1]">
				...
				<xsl:when test="exslt:node-set($tokens)/token[1] = '&lt;'">
				  <xsl:choose>
				    <xsl:when test="exslt:node-set($tokens)/token[2] = '='">
    					<xsl:text> &lt;= </xsl:text>
    					<xsl:call-template name="exslt:replace">
    						<xsl:with-param name="tokens">
    							<xsl:for-each select="exslt:node-set($tokens)/token[position() > 2]">
    								<xsl:copy-of select="."/>
    							</xsl:for-each>
    						</xsl:with-param>
    					</xsl:call-template>
				...
				<xsl:when test="contains(exslt:node-set($tokens)/token[1], ':pow')">
				...
				<xsl:when test="contains(exslt:node-set($tokens)/token[1], ':log')">
					<xsl:variable name="comma-location">
						<xsl:call-template name="exslt:find-token">
							<xsl:with-param name="tokens">
								<xsl:for-each select="exslt:node-set($tokens)/token[position() > 2]">
									<xsl:copy-of select="."/>
								</xsl:for-each>
					...
					<xsl:choose>
						<xsl:when test="$comma-location = 2 and exslt:node-set($tokens)/token[3] = '10'">
							<xsl:text>$log10(</xsl:text>
							<xsl:call-template name="exslt:replace">
								<xsl:with-param name="tokens">
									<xsl:for-each select="exslt:node-set($tokens)/token[position() > ($comma-location + 2) and position() &lt; ($end-bracket-location + 2)]">
										<xsl:copy-of select="."/>
									</xsl:for-each>
								</xsl:with-param>
							</xsl:call-template>
						...
						<xsl:otherwise>
							...
							<xsl:call-template name="exslt:replace">
								<xsl:with-param name="tokens">
									<xsl:for-each select="exslt:node-set($tokens)/token[position() > 2 and position() &lt; ($comma-location + 2)]">
										<xsl:copy-of select="."/>
									</xsl:for-each>
								</xsl:with-param>
							</xsl:call-template>
					...
					<xsl:call-template name="exslt:replace">
						<xsl:with-param name="tokens">
							<xsl:for-each select="exslt:node-set($tokens)/token[position() > $end-bracket-location + 2]">
								<xsl:copy-of select="."/>
							</xsl:for-each>
						</xsl:with-param>
					</xsl:call-template>
				</xsl:when>
				<xsl:when test="exslt:node-set($tokens)/token[1] = 'or'">
					<xsl:text> || </xsl:text>
					<xsl:call-template name="exslt:replace">
						<xsl:with-param name="tokens">
							<xsl:for-each select="exslt:node-set($tokens)/token[position() > 1]">
				...
				<xsl:when test="contains(exslt:node-set($tokens)/token[1], ':decode')">
					<xsl:choose>
						<xsl:when test="exslt:node-set($tokens)/token[3] = $apos or exslt:node-set($tokens)/token[3] = $quot">
							<xsl:call-template name="parse-scale">
								<xsl:with-param name="value" select="exslt:node-set($tokens)/token[4]"/>
							</xsl:call-template>
							<xsl:call-template name="add-space">
								<xsl:with-param name="next-token" select="exslt:node-set($tokens)/token[7]"/>
							</xsl:call-template>
						...
						<xsl:when test="exslt:node-set($tokens)/token[3] = 'id'">
							<xsl:call-template name="exslt:replace">
								<xsl:with-param name="tokens">
									<xsl:for-each select="exslt:node-set($tokens)/token[position() > 2 and position() &lt; 9]">
										<xsl:copy-of select="."/>
									</xsl:for-each>
								</xsl:with-param>
							</xsl:call-template>
							...
							<xsl:call-template name="exslt:replace">
								<xsl:with-param name="tokens">
									<xsl:for-each select="exslt:node-set($tokens)/token[position() > 9]">
										<xsl:copy-of select="."/>
									</xsl:for-each>
								</xsl:with-param>
							</xsl:call-template>
				...
				<xsl:when test="exslt:node-set($tokens)/token[1] = 'concat'">
					<xsl:text>{</xsl:text>
					...
					<xsl:call-template name="exslt:replace">
						<xsl:with-param name="tokens">
							<xsl:for-each select="exslt:node-set($tokens)/token[position() > 2 and position() &lt; ($location + 2)]">
								<xsl:copy-of select="."/>
							</xsl:for-each>
						</xsl:with-param>
					</xsl:call-template>
					<xsl:text>}</xsl:text>
					<xsl:call-template name="add-space">
						<xsl:with-param name="next-token" select="exslt:node-set($tokens)/token[$location + 3]"/>
					</xsl:call-template>
					<xsl:call-template name="exslt:replace">
						<xsl:with-param name="tokens">
							<xsl:for-each select="exslt:node-set($tokens)/token[position() > ($location + 2)]">
								<xsl:copy-of select="."/>
							</xsl:for-each>
						</xsl:with-param>
					</xsl:call-template>
				...
				<xsl:otherwise>
					<xsl:choose>
						<xsl:when test="exslt:node-set($tokens)/token[2] = '='">
							<xsl:value-of select="exslt:node-set($tokens)/token[1]"/>
							<xsl:choose>
								<xsl:when test="contains(exslt:node-set($tokens)/token[1], '!') or contains(exslt:node-set($tokens)/token[1], '>') or contains(exslt:node-set($tokens)/token[1], '&lt;')">
									<xsl:text>=</xsl:text>
									<xsl:call-template name="add-space">
										<xsl:with-param name="next-token" select="exslt:node-set($tokens)/token[3]"/>
									</xsl:call-template>
									<xsl:call-template name="exslt:replace">
										<xsl:with-param name="tokens">
											<xsl:for-each select="exslt:node-set($tokens)/token[position() > 2]">
						...
						<xsl:otherwise>
							<xsl:choose>
								<xsl:when test="exslt:node-set($tokens)/token[1] = ')'">
									<xsl:value-of select="exslt:node-set($tokens)/token[1]"/>
									<xsl:call-template name="add-space">
										<xsl:with-param name="next-token" select="exslt:node-set($tokens)/token[2]"/>
									</xsl:call-template>
								</xsl:when>
								<xsl:when test="exslt:node-set($tokens)/token[1] = '(' or exslt:node-set($tokens)/token[2] = ')' or exslt:node-set($tokens)/token[2] = ',' or exslt:node-set($tokens)/token[1] = $apos or exslt:node-set($tokens)/token[1] = $quot">
									<xsl:value-of select="exslt:node-set($tokens)/token[1]"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="exslt:node-set($tokens)/token[1]"/>
	...
	<xsl:template name="exslt:find-token">
		<xsl:param name="tokens"/>
		<xsl:param name="token"/>
		<xsl:param name="index" select="1"/>
		<xsl:param name="level" select="0"/>
		<xsl:if test="debug">
			<xsl:message>find-token(<xsl:value-of select="exslt:node-set($tokens)"/>, <xsl:value-of select="$token"/>, <xsl:value-of select="$index"/>)</xsl:message>
		</xsl:if>
		<xsl:if test="$level >= 0 and count(exslt:node-set($tokens)/token) > 0">
			<xsl:choose>
				<xsl:when test="$level = 0 and exslt:node-set($tokens)/token[$index] = $token">
					<xsl:value-of select="$index"/>
				</xsl:when>
				<xsl:when test="exslt:node-set($tokens)/token[$index] = '('">
					<xsl:call-template name="exslt:find-token">
						<xsl:with-param name="tokens" select="exslt:node-set($tokens)"/>
